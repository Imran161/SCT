...
from segmentation import save_to_papki

from sct_val import test_model
from utils import SCT_base_classes, SCT_out_classes, \
binary_cross_entropy, focal_loss, iou_metric, \
weak_iou_loss, strong_iou_loss, weak_combined_loss, strong_combined_loss, \
ImageVisualizer, ExperimentSetup
...
# было так, норм работал но каждая папка делится, а надо чтобы каждая папка шла либо в трейн либо в вал
# def make_dataloaders(subdirectories_list, batch_size): # probs
#     all_train_data = []
#     all_val_data = []
    
#     count = 0
#     for s in subdirectories_list:                                       #тут разделить папки сразу
#         sub_subdirectories_list = get_direct_subdirectories(s)  
        
        
#         # for count, i in enumerate(sub_subdirectories_list):
#         for i in sub_subdirectories_list:
#             try:
#                 # first_number, second_chislo = get_number(i)

#                 sct_coco = convert_from_coco(i)    # probs              #сюда либо 0 либо 100 посылать в probs
...
#                 # print("pixel_TotalTrain shape", pixel_TotalTrain.shape)
                
#                 train_dataset = Subset(sct_coco, sct_coco.train_list)
#                 # print("len sct_coco.train_list", len(sct_coco.train_list))
#                 val_dataset = Subset(sct_coco, sct_coco.val_list)
#                 # print("len sct_coco.val_list", len(sct_coco.val_list))
...
#     # print("all_train_data", all_train_data)
...
#     train_loader = DataLoader(concat_train_data, batch_size=batch_size, shuffle=True, num_workers=4)
#     val_loader = DataLoader(concat_val_data, batch_size=batch_size, shuffle=False, num_workers=4)
    
#     return train_loader, val_loader, TotalTrain, pixel_TotalTrain, sct_coco.list_of_name_out_classes
...
class COCODataLoader:
    ...
    def make_dataloaders(self, subdirectories_list):
        ...
        train_loader = DataLoader(concat_train_data, batch_size=self.batch_size, shuffle=True, num_workers=4)
        val_loader = DataLoader(concat_val_data, batch_size=self.batch_size, shuffle=False, num_workers=4)
...
def train_model(model,
    ...
    for epoch in range(num_epochs):
        ...
        for class_idx, iou_value in enumerate(train_iou_avg):
            class_name = class_names_dict[class_idx + 1]  # в out classes индексы с единицы
    ...
    # тут сохранение лучшей модели
        if val_loss_avg < best_loss:
            ...
            torch.save(model.state_dict(), f"{best_model_path}/best_{experiment_name}_model.pth")
    
    last_model_path = "last_models"
    if not os.path.exists(last_model_path):
                os.makedirs(last_model_path)
                
    torch.save(model.state_dict(), f"{last_model_path}/last_{experiment_name}_model.pth")   
...
if __name__ == "__main__": 
    ...
    train_loader, val_loader, TotalTrain, pixel_TotalTrain, list_of_name_out_classes = make_dataloaders(subdirectories_list, batch_size)    
