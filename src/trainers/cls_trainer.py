...
class ModelTrainer:
    def __init__(self, model, optimizer, criterion, num_epochs, train_loader, val_loader, device,
                 num_classes, experiment_name, all_class_weights=None, alpha=None, use_opt_pixel_weight=False):
        self.model = model
        ...
        self.train_predict_path = f"/home/imran-nasyrov/sct_project/sct_data/predict_test/train"
        self.train_image_visualizer = ImageVisualizer(self.train_predict_path)
        
        self.val_predict_path = f"/home/imran-nasyrov/sct_project/sct_data/predict_test/val"
        ...
        self.metrics_calculator = Detection_metrics(mode="ML", num_classes=self.num_classes)
        
        self.class_names_dict = {class_info['id']: class_info['name'] for class_info in SCT_out_classes}
        ...
        for epoch in range(self.num_epochs):
            ...
            with tqdm(total=len(self.train_loader), desc=f'Epoch {epoch + 1}/{self.num_epochs}', unit='batch') as pbar:
                for train_batch in self.train_loader:
                    ...
                    loss = self.criterion(outputs, masks, [x[1:] for x in self.all_class_weights], self.alpha)
                    loss.backward()
                    self.optimizer.step()
            ...
            print(f"Epoch [{epoch+1}/{self.num_epochs}], Train Loss: {train_loss_avg}, Train IoU: {train_iou_avg}")
        ...
        for epoch in range(self.num_epochs):
            ...
            with torch.no_grad():
                for val_batch in self.val_loader: 
                    ...
                    val_loss_sum += self.criterion(outputs_val, masks_val, None, None).item()
            ...
            print(f"Epoch [{epoch+1}/{self.num_epochs}], Val Loss: {val_loss_avg},  Val IoU: {val_iou_avg}")
