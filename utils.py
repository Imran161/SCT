...
def binary_cross_entropy(outputs: torch.Tensor, labels: torch.Tensor) -> torch.Tensor:
    loss = -labels * torch.log(outputs + 0.00001) - (1 - labels) * torch.log(1 - outputs + 0.00001)
    return loss
    ...
    if alpha is not None:    
        for i in range(size[0]):
            for j in range(size[1]):
                weight_matrix = (target[i, j]) * alpha[0][j]+ \
                                ((1-target[i, j])) * alpha[1][j]
...
def weak_iou_loss(outputs: torch.Tensor, labels: torch.Tensor, class_weight=None):
    ...
    intersection = (outputs * labels).float().sum((-1, -2))  # Вычисляем пересечение для каждого изображения
    #union = (outputs+labels - outputs*labels).float().sum((-1, -2)) не недо так делать то есть считать два раза может градиент глюкануть
    
        
    union = ((outputs+labels).float().sum((-1, -2)) - intersection).sum((-1, -2)) # возможно тут float и не нужен
    iou = 1 - (intersection) / (union + SMOOTH)  # Вычисляем IoU для каждого изображения
...
class ImageVisualizer:
    ...
    def visualize(self, images, true_masks, pred_masks, class_names_dict, colors, epoch = None):
        ...
        for i in range(len(images)):
            ...
            true_image_with_contours = np.zeros((image.shape[0], image.shape[1], 3), dtype=np.uint8)
            true_image_with_contours[:, :, 0] = image  # Копируем одноканальное изображение в канал синего цвета (Blue)
            true_image_with_contours[:, :, 1] = image  # Копируем одноканальное изображение в канал зеленого цвета (Green)
            true_image_with_contours[:, :, 2] = image  # Копируем одноканальное изображение в канал красного цвета (Red)
            ...
            for j in range(np.shape(true_masks[i])[0]):
                ...
                contours, _ = cv2.findContours(true_masks[i][j].astype(int).astype(np.uint8), cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)
                true_image_with_contours = cv2.drawContours(true_image_with_contours, contours, -1, color, 2)
                ...
                #     # text = list_of_name_out_classes[j]
                    ...
                    cv2.putText(true_image_with_contours, f"true class: {text}", (10, 20), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 1, cv2.LINE_AA) 
                # k+=50
            ...
            pred_image_with_contours = np.zeros((image.shape[0], image.shape[1], 3), dtype=np.uint8)
            pred_image_with_contours[:, :, 0] = image  # Копируем одноканальное изображение в канал синего цвета (Blue)
            pred_image_with_contours[:, :, 1] = image  # Копируем одноканальное изображение в канал зеленого цвета (Green)
            pred_image_with_contours[:, :, 2] = image  # Копируем одноканальное изображение в канал красного цвета (Red)

            for j in range(np.shape(pred_masks[i])[0]):
                color = (colors[j][0][0], colors[j][0][1], colors[j][0][2])
                contours, _ = cv2.findContours(pred_masks[i][j].astype(int).astype(np.uint8), cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)
                # print("contours", contours)
                pred_image_with_contours = cv2.drawContours(pred_image_with_contours, contours, -1, color, 2)
                ...
                if np.max(pred_masks[i][j])==1:
                    ...
                    # print("text", text)
                    # plt.text(2000, k, text , color = (self.colors[i][0][0]/255, self.colors[i][0][1]/255, self.colors[i][0][2]/255)) 
                    cv2.putText(pred_image_with_contours, f"pred class: {text}", (10, 20), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 1, cv2.LINE_AA) 
                        # k+=50
            ...
            combined_image = np.concatenate((true_image_with_contours, pred_image_with_contours), axis=1)
            ...
            if len(values) > 1:
                if epoch is None:
                    ...
                    cv2.imwrite(f"{self.output_path}/epoch_{epoch}_image_{i}.jpg", combined_image)



class ExperimentSetup:
    def __init__(self, train_loader: DataLoader, TotalTrain: np.ndarray, pixel_TotalTrain: np.ndarray,  batch_size: int, num_classes:int, use_background = False) -> None:
    ...
    def setup_experiment(self, use_class_weight: bool, use_pixel_weight: bool, use_pixel_opt: bool, power: str) -> Tuple[Optional[List[float]], Optional[List[float]], str, Any]:    
        if use_class_weight == False:
            ...
            class_weights = self.TotalTrain[1:].sum()/(self.TotalTrain * self.num_classes) # MC
            class_weight_use_background = self.TotalTrain.sum()/(self.TotalTrain * self.num_classes + 1) # это если вдруг с фоном веса нужны 
        ...
        if use_pixel_weight == False:
            ...
            pixel_neg_weights = pixel_count_data / (2 * (pixel_count_data - self.pixel_TotalTrain))
            # pixel_class_weights = pixel_count_data / (self.num_classes * self.pixel_TotalTrain) 
            pixel_class_weights = self.pixel_TotalTrain[1:].sum()/(self.pixel_TotalTrain * self.num_classes)
            pixel_class_weight_use_background = self.pixel_TotalTrain.sum()/(self.pixel_TotalTrain * self.num_classes + 1)
            
            pixel_all_class_weights = [pixel_pos_weights, pixel_neg_weights] #, pixel_class_weights]
