...
class Universal_json_Segmentation_Dataset:
    ...
    def check_create_train_val_list(self, recalculate):
        try:
            train_list = open(
                self.json_file_path +
                f"train_list_{self.out_cl_hash}.pickle", "rb"
            )
            val_list = open(
                self.json_file_path +
                f"val_list_{self.out_cl_hash}.pickle", "rb"
            )
            all_img_list = open(
                self.json_file_path +
                f"all_img_list_{self.out_cl_hash}.pickle", "rb"
            )
            ...
            for img_index in imgIds:
                anns_ids = self.coco.getAnnIds(
                    imgIds=img_index, catIds=self.catIDs)
            ...
            with open(
                self.json_file_path +
                    f"train_list_{self.out_cl_hash}.pickle", "wb"
            ) as train_list:
                pickle.dump(self.train_list, train_list)
            with open(
                self.json_file_path +
                    f"val_list_{self.out_cl_hash}.pickle", "wb"
            ) as val_list:
                pickle.dump(self.val_list, val_list)
            with open(
                self.json_file_path +
                    f"all_img_list_{self.out_cl_hash}.pickle", "wb"
            ) as all_img_list:
    ...
    def check_create_weight(self, recalculate):
        try:
            TotalTrain = open(
                self.json_file_path +
                f"TotalTrain_{self.out_cl_hash}.pickle", "rb"
            )
            TotalVal = open(
                self.json_file_path +
                f"TotalVal_{self.out_cl_hash}.pickle", "rb"
            )
            pixel_TotalTrain = open(
                self.json_file_path +
                f"pixel_TotalTrain_{self.out_cl_hash}.pickle",
                "rb",
            )
            pixel_TotalVal = open(
                self.json_file_path +
                f"pixel_TotalVal_{self.out_cl_hash}.pickle", "rb"
            )
            ...
            with open(
                self.json_file_path +
                    f"TotalTrain_{self.out_cl_hash}.pickle", "wb"
            ) as Total:
                pickle.dump(self.TotalTrain, Total)
            with open(
                self.json_file_path +
                    f"TotalVal_{self.out_cl_hash}.pickle", "wb"
            ) as Total:
            ...
            with open(
                self.json_file_path +
                    f"pixel_TotalVal_{self.out_cl_hash}.pickle", "wb"
            ) as pix_Total:
    ...
    def show_me_contours(self, idx):
        ...
        for i in range(np.shape(mask)[0]):
            ...
            rgb_image = cv2.drawContours(
                ...
                contours,
                -1,
                (self.colors[i][0][0], self.colors[i]
                 [0][1], self.colors[i][0][2]),
                2,
            )
        ...
        if not os.path.exists(
            ...
            os.mkdir(
                "/home/imran-nasyrov/sct_project/sct_data/output_images/" +
                text[0]
            )
    ...
    def __getitem__(self, idx, contures=False):
        ...
        image_path = self.json_file_path + \
            "images/" + images_description["file_name"]
        # print("image_path", image_path)
        rgb_image = cv2.imread(image_path)
        gray_image = cv2.cvtColor(rgb_image, cv2.COLOR_BGR2GRAY)

        anns_ids = self.coco.getAnnIds(
            imgIds=idx, catIds=self.catIDs, iscrowd=None)
        anns = self.coco.loadAnns(anns_ids)
        ...
        if self.resize is not None and not contures:  # без not и == False было
            ...
            if self.dataloader == False:
                ...
                image = (image - image.min()) / \
                    (image.max() - image.min() + 0.00000001)
...
class Universal_npz_Segmentation_Dataset:
    ...
    def check_create_data(self, recalculate):
        try:
            ...
            self.all_images_and_mask_paths = pickle.load(
            ...
            with open(
                ...
                pickle.dump(self.all_images_and_mask_paths,
                            all_images_and_mask_paths)
    ...
    def load_data(self, path_to_file):
        if path_to_file == None:
            data = np.zeros(
                (len(self.base_classes), self.resize[0], self.resize[1]))
        else:
    ...
    def __getitem__(self, idx, for_inner_use=False):
        ...
        mask = torch.zeros(
            ...
            1, base_mask.size()[-2], base_mask.size()[-1]
        )
        mask[1:, :, :] = base_mask
        mask = self.to_out_classes(mask)
...
SCT_out_classes = [
    {"id": 1, "name": "insult_type_1",
        "summable_masks": [1], "subtractive_masks": []},
    {"id": 2, "name": "insult_type_2",
        "summable_masks": [2], "subtractive_masks": []},
    {"id": 3, "name": "insult_type_3",
        "summable_masks": [3], "subtractive_masks": []},
    {"id": 4, "name": "insult_type_4",
        "summable_masks": [4], "subtractive_masks": []},
]


SCT_pat = [
    {"id": 1, "name": "1", "summable_masks": [
        1, 2, 3, 4, 5], "subtractive_masks": [4]},
]
