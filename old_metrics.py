...
class Detection_metrics:
    def __init__(self, mode, num_classes, treshold=0.5):
        ...
        self.Average_IOU = np.zeros(
            self.num_classes,
        ).astype(np.float32)
        self.Average_tp = np.zeros(
            self.num_classes,
        ).astype(np.float32)
        self.Average_fp = np.zeros(
            self.num_classes,
        ).astype(np.float32)
        self.Average_fn = np.zeros(
            self.num_classes,
        ).astype(np.float32)
    ...
    def calculate_confidence(self, prob_masks):
        ...
        for i in range(prob_masks.size()[0]):
            loss = (prob_masks[i] * torch.log(prob_masks[i] + smooth)).sum() / (
                prob_masks[i].sum() + smooth
            )
            loss += (
                (1 - prob_masks[i]) * torch.log(1 - prob_masks[i] + smooth)
            ).sum() / ((1 - prob_masks[i]).sum() + smooth)
            loss /= 2
            confidence = loss.exp()
        ...
        for class_num in range(class_nums):
            ...
            with warnings.catch_warnings():
                ...
                if confusion_matrix[1, 1] + confusion_matrix[1, 0] == 0:
                    ...
                    precession = confusion_matrix[0, 0] / (
    ...
    def update_counter(self, true_mask, pred_mask):
        ...
        Average_batch_IOU = np.zeros(
            self.num_classes,
        ).astype(np.float32)
        Average_batch_tp = np.zeros(
            self.num_classes,
        ).astype(np.float32)
        Average_batch_fp = np.zeros(
            self.num_classes,
        ).astype(np.float32)
        Average_batch_fn = np.zeros(
            self.num_classes,
        ).astype(np.float32)
        ...
        for i in range(batch_size):
            for j in range(self.num_classes):
                (
                    ...
                    instance_fp,
                    instance_fn,
                ) = self.easy_detect_objects(true_mask[i, j], pred_mask[i, j])
    ...
    def calculate_metrics(self):
        ...
        (
            area_probs_AUROC,
            area_probs_recall,
            area_probs_precession,
            area_probs_F1,
        ) = self.calculate_AUROC(self.all_true_label, self.all_confidences)

        (
            confidence_AUROC,
            confidence_recall,
            confidence_precession,
            confidence_F1,
        ) = self.calculate_AUROC(self.all_true_label, self.all_probs)
